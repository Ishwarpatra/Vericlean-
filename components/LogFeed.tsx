  ...
  const getStatusBadge = (status: LogStatus) => {
    switch (status) {
      case LogStatus.VERIFIED:
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800"><Check size={10} className="mr-1"/> Verified</span>;
      case LogStatus.REJECTED:
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800"><X size={10} className="mr-1"/> Rejected</span>;
      case LogStatus.FLAGGED:
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800"><AlertOctagon size={10} className="mr-1"/> Review</span>;
      default:
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 h-full flex flex-col">
      <div className="p-4 border-b border-gray-100 flex justify-between items-center">
        <h3 className="font-semibold text-gray-900">Live Cleaning Feed</h3>
      ...
      <div className="overflow-y-auto flex-1 p-2 space-y-2 scrollbar-hide">
        {logs.length === 0 && (
            <div className="text-center py-10 text-gray-400 text-sm">No activity yet today.</div>
        )}
        {logs.map((log) => (
          <div 
            key={log.id} 
            onClick={() => onSelectLog(log)}
            className="group p-3 rounded-lg hover:bg-gray-50 border border-transparent hover:border-gray-200 transition-all cursor-pointer flex items-start gap-3"
          >
            <div className="mt-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start">
                ...
                <span className="text-xs text-gray-400 whitespace-nowrap ml-2">{formatTime(log.created_at)}</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                {log.proof_of_quality && (
                  <span className={`text-xs font-mono font-medium ${log.proof_of_quality.overall_score > 80 ? 'text-green-600' : 'text-amber-600'}`}>
